...
class Conv:
  def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True) -> None:
    ...
    self.bias:Optional[Tensor] = Tensor.uniform(out_channels, low=-self.bound, high=self.bound) if bias else None
...
class Linear:
  def __init__(self, in_features:int, out_features:int, bias:bool=True) -> None:
    ...
    self.bias:Optional[Tensor] = Tensor.uniform(self.out_features, low=-self.bound, high=self.bound) if bias else None
...
class Embedding:
  ...
  def __call__(self, idx:Tensor, **kwargs) -> Tensor:
