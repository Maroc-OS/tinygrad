...
if __name__ == "__main__":
  with Tensor.no_grad():
    ...
    with Timing("create model: "):
      model = Transformer(4096, 14336, n_heads=32, n_layers=32, norm_eps=1e-5, vocab_size=32002, n_kv_heads=8, max_context=4096, jit=bool(getenv("JIT", True)))
    ...
    while 1:
      ...
      while 1:
        ...
        start_pos = len(toks)
        toks.append(tok)
        outputted = output(outputted, toks, "cyan" if turn else "blue")
        if tok == IM_END: break
